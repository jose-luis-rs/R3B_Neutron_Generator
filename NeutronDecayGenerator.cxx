...
double Cnn_0(double X, double r0) { // For nn-FSI in direct decay
  ...
  if (r0 == 0.0) {
    ...
    if (X == 0.) {
      ...
      F1 = (TMath::Erf(2. * r0 * X / hc) * sqrt(pi) / 2.) *
           exp(-4. * (r0 * X / hc) * (r0 * X / hc)) / (2. * r0 * X / hc);
      F2 = (1. - exp(-4. * (r0 * X / hc) * (r0 * X / hc))) / (2. * r0 * X / hc);
    }
    Bi = 0.25 * f_2 / (r0 * r0) * (1. - d0 / (2. * sqrt(pi) * r0)) +
         Ref * F1 / (sqrt(pi) * r0) - Imf * F2 / (2. * r0);
    res = 1. + B0 + Bi;
  }
  return res;
}
...
double Integrate_BI(double r0, double t0, double G, double Mv, double rho,
  ...
  for (i = 1; i <= N; i++) {
    ...
    for (j = 1; j <= N; j++) {
      xn = x1 + j * step - step / 2.;
      intX = intX +
             intBi(xn, r0, t0, rT, G, Mv, rho, f_2, k, Imf, qT, q0, Ref) * step;
    }
...
double Cnn_sequential(double *p1, double *p2, double r0,
  ...
  if (r0 == 0.) {
    ...
    pi = acos(-1.);
    q = TMath::Sqrt((p1[0] - p2[0]) * (p1[0] - p2[0]) +
                    (p1[1] - p2[1]) * (p1[1] - p2[1]) +
                    (p1[2] - p2[2]) * (p1[2] - p2[2]));
    ...
    qT =
        TMath::Sqrt(TMath::Abs(4. * k * k - (q0 / (G * Mv)) * (q0 / (G * Mv))));
    ...
    B0 = -0.5 *
         exp(-(r0 * q / hc) * (r0 * q / hc) - (t0 * q0 / hc) * (t0 * q0 / hc));
...
Estar(double mn, double mf, double *Pnn1, double *Pnn2,
  ...
  res = TMath::Sqrt(mf * mf + mn * mn + 2 * Pnn[3] * Pnn3[3] -
                    2 * VoV(Pnn, Pnn3)) -
        mf - mn;
  return res;
}
...
void PHASE_3(double a1, double a2, double a3, double Ed, double *w1, double *w2,
  ...
  E = a1 + a2 + a3 + Ed;
  p3M = sqrt((E * E - (a1 + a2 + a3) * (a1 + a2 + a3)) *
             (E * E - (a1 + a2 - a3) * (a1 + a2 - a3))) /
        (2. * E);
  Fold = 0.;
  step = p3M / Ndiv;
  Pmax = p3M - step;
  ...
  p1 = sqrt(fabs((M12 * M12 - (a1 + a2) * (a1 + a2)) *
                 (M12 * M12 - (a1 - a2) * (a1 - a2)))) /
       (2. * M12);
  ran[0] = Rand_angle->Uniform(0., 1.);
  ran[1] = Rand_angle->Uniform(0., 1.);
...
double R_2(double a1, double a2, double Ed, double Er) {
  ...
  if (E2 >= Ex) {
    ...
    E = m1 + m2 + Ex;
    R_0 = TMath::Sqrt((E * E - (m2 - m1) * (m2 - m1)) *
                      (E * E - (m2 + m1) * (m2 + m1))) /
          (E * E);
    res = TMath::Sqrt((E * E - ((m2 + E2) - m1) * ((m2 + E2) - m1)) *
                      (E * E - ((m2 + E2) + m1) * ((m2 + E2) + m1))) /
          (E * E) / R_0;
  }
  return res;
}
...
PHASE2x2(double a1, double a2, double a3, double Ed, double *w1, double *w2,
  ...
  M12 = m1 + m2 + Ex;
  Pcm = TMath::Sqrt(TMath::Abs((M12 * M12 - (m1 + m2) * (m1 + m2)) *
                               (M12 * M12 - (m1 - m2) * (m1 - m2)))) /
        (2. * M12);
  ran[0] = Rand_angle->Uniform(0., 1.);
  ran[1] = Rand_angle->Uniform(0., 1.);
  ...
  Ex = E23;
  M12 = m1 + m2 + Ex;
  Pcm = sqrt(TMath::Abs((M12 * M12 - (m1 + m2) * (m1 + m2)) *
                        (M12 * M12 - (m1 - m2) * (m1 - m2)))) /
        (2. * M12);
  ran[0] = Rand_angle->Uniform(0., 1.);
  ran[1] = Rand_angle->Uniform(0., 1.);
...
double DALITZ(double m_j, double m_i, double Ed, double *P_i, double *P_j) {
  ...
  res = (M_ij - (m_i + m_j) * (m_i + m_j)) /
        ((m_i + m_j + Ed) * (m_i + m_j + Ed) - (m_i + m_j) * (m_i + m_j));
  return res;
}
...
void EventGenerator_Ndecay(const TString Output_Name = "test",
  ...
  for (Long64_t i = 0; i < Nb; i++) {
    ...
    if (N == 2) {
      ...
      if (FSI_nn == 1) {
        do {
          if (SEQ_Decay == 1) { // Sequential decay mode
            E23 = PHASE2x2(mn, mn, mf, Ed, Pnn1, Pnn2, Pnn3, Rand_angle, A,
                           ESeq_BW);
            ...
            E23 = PHASE2x2(mf, mn, mn, Ed, Pnn3, Pnn2, Pnn1, Rand_angle, A,
                           BW_Dineutron);
      ...
      P_n1[3] = TMath::Sqrt(mn * mn + P_n1[0] * P_n1[0] + P_n1[1] * P_n1[1] +
                            P_n1[2] * P_n1[2]);
      ...
      P_n2[3] = TMath::Sqrt(mn * mn + P_n2[0] * P_n2[0] + P_n2[1] * P_n2[1] +
                            P_n2[2] * P_n2[2]);
      ...
      P_f[3] = TMath::Sqrt(mf * mf + P_f[0] * P_f[0] + P_f[1] * P_f[1] +
                           P_f[2] * P_f[2]);
      ...
      P_tot[3] =
          TMath::Sqrt((mf + 2 * mn) * (mf + 2 * mn) + P_tot[0] * P_tot[0] +
                      P_tot[1] * P_tot[1] + P_tot[2] * P_tot[2]);
      ...
      ky_Y_1[0] =
          (mn * (P_f[0] + P_n1[0]) - (mn + mf) * P_n2[0]) / (2 * mn + mf);
      ky_Y_1[1] =
          (mn * (P_f[1] + P_n1[1]) - (mn + mf) * P_n2[1]) / (2 * mn + mf);
      ky_Y_1[2] =
          (mn * (P_f[2] + P_n1[2]) - (mn + mf) * P_n2[2]) / (2 * mn + mf);

      ky_T_1[0] =
          (mf * (P_n1[0] + P_n2[0]) - (mn + mn) * P_f[0]) / (2 * mn + mf);
      ky_T_1[1] =
          (mf * (P_n1[1] + P_n2[1]) - (mn + mn) * P_f[1]) / (2 * mn + mf);
      ky_T_1[2] =
      ...
      ky_Y_2[0] =
          (mn * (P_f[0] + P_n2[0]) - (mn + mf) * P_n1[0]) / (2 * mn + mf);
      ky_Y_2[1] =
          (mn * (P_f[1] + P_n2[1]) - (mn + mf) * P_n1[1]) / (2 * mn + mf);
      ky_Y_2[2] =
          (mn * (P_f[2] + P_n2[2]) - (mn + mf) * P_n1[2]) / (2 * mn + mf);

      ky_T_2[0] =
          (mf * (P_n2[0] + P_n1[0]) - (mn + mn) * P_f[0]) / (2 * mn + mf);
      ky_T_2[1] =
          (mf * (P_n2[1] + P_n1[1]) - (mn + mn) * P_f[1]) / (2 * mn + mf);
      ky_T_2[2] =
      ...
      if (m_nn->size() != m_fn1->size() || m_nn->size() != m_fn2->size() ||
          m_nn->size() != E_star2n->size()) {
